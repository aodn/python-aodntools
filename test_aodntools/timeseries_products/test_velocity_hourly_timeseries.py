...
from aodntools.timeseries_products.velocity_hourly_timeseries import (
...
class TestVelocityHourlyTimeseries(BaseTestCase):
    EXPECTED_OUTPUT_FILE = os.path.join(
        TEST_ROOT,
        "IMOS_ANMN-NRS_VZ_20180816_NRSROT_FV02_velocity-hourly-timeseries_END-20191018_C-20220608.nc",
    )

    def test_velocity_hourly(self):
        output_file, bad_files = velocity_hourly_aggregated(
            INPUT_FILES, "NRSROT", input_dir=TEST_ROOT, output_dir="/tmp"
        )
        ...
        self.assertSetEqual(
            set(dataset.variables.keys()), OBS_VARS | INST_VARS | STR_VARS
        )
