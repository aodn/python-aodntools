...
class TestImosTemplate(TemplateTestCase):
    ...
    def test_combine_global_attributes(self):
        my_globals = {"title": "This is a test", "project": "Test project"}
    ...
    def test_add_date_created_attribute(self):
        ...
        parsed_date_created = datetime.strptime(
            self.template.global_attributes["date_created"], "%Y-%m-%dT%H:%M:%SZ"
        )
        self.assertTrue(
            self.template.date_created - parsed_date_created < timedelta(seconds=1)
        )

    def test_add_extent_attributes(self):
        self.template.add_extent_attributes(
            time_var=None, vert_var=None, lat_var=None, lon_var=None
        )
        for att in (
            "time_coverage_start",
            "time_coverage_end",
        ...
        self.template.variables = {
            ...
            "LATITUDE": {
                ...
                "_datatype": "float32",
                "_FillValue": -999.0,
                "_data": np.array([-999.0, -999.0, -42, -43, 12]),
            },
            "LONGITUDE": {
                "_dimensions": ["TIME"],
                "_datatype": "float32",
            ...
            "NOMINAL_DEPTH": {"_datatype": "float32", "_data": 20},
            "DEPTH": {
                "_dimensions": ["TIME"],
                "_datatype": "float32",
        ...
        self.assertEqual(
            ...
            self.template.global_attributes["time_coverage_start"],
        )
        self.assertEqual(
            "2019-03-01T09:00:00Z", self.template.global_attributes["time_coverage_end"]
        )
