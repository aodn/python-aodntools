...
TEMPLATE_JSON = resource_filename(
    __name__, "velocity_aggregated_timeseries_template.json"
)
    ...
    for index, file in enumerate(files_to_agg):
        ...
        with xr.open_dataset(os.path.join(input_dir, file)) as nc:
            ...
            if "HEIGHT_ABOVE_SENSOR" in nc.dims:
                ...
                DEPTHqc[start:end] = np.repeat(
                    flat_variable(nc, "DEPTH_quality_control"), n_cells
                )
                CELL_INDEX[start:end] = np.tile(
                    np.arange(n_cells, dtype=np.uint32), n_measurements
                )
            ...
            TIME[start:end] = (
                np.repeat(flat_variable(nc, "TIME"), n_cells) - epoch
            ) / one_day
