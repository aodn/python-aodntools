        ...
        with xr.open_dataset(os.path.join(input_dir, file)) as nc:
            ...
            if error_list:
                ...
                file_list_dataframe = file_list_dataframe.append(
                    {"url": file, "deployment_date": parse(nc.time_deployment_start)},
                    ignore_index=True,
                )
...
def get_parameter_names(nc):
    ...
    """
    params = list(
        set([s.strip("_quality_control") for s in get_qc_variable_names(nc)])
        - set(list(nc.coords))
    )
    return params
...
def good_data_only(nc, qcflags):
    ...
    nc_masked = (
        nc[varnames[0]]
        .where(nc[varnames[0] + "_quality_control"].isin(qcflags))
        .to_dataset(name=varnames[0])
    )
    for variable in varnames[1:]:
        nc_masked[variable] = nc[variable].where(
            nc[variable + "_quality_control"].isin(qcflags)
        )
...
def get_QCcount(nc, qcflags):
    ...
    if 0 in qcflags and len(qcflags) > 1:
        ...
        for variable in varnames:
            ...
            for flag in qcflags:
                flag_count.append(
                    int(np.sum(nc[variable + "_quality_control"] == flag))
                )
    ...
    if len(qc_count) > 0:
        for variable in qc_count.keys():
            if qc_count[variable]["qcnon0_count"] > 0:
                qc_percent[variable] = {
                    "percent_quality_controlled": round(
                        100
                        * (
                            1
                            - qc_count[variable]["qc0_count"]
                            / (
                                qc_count[variable]["qcnon0_count"]
                                + qc_count[variable]["qc0_count"]
                            )
                        ),
                        2,
...
def set_globalattr(
    nc_aggregated, templatefile, site_code, add_attribute, parameter_names
):
    """
    ...
    agg_attr = {
        "title": (
            "Long time series Hourly Aggregated product: all available non-velocity variables at "
            + site_code
            + " between "
            + pd.to_datetime(nc_aggregated.TIME.values.min()).strftime(timeformat)
            + " and "
            + pd.to_datetime(nc_aggregated.TIME.values.max()).strftime(timeformat)
        ),
        "site_code": site_code,
        "time_coverage_start": pd.to_datetime(nc_aggregated.TIME.values.min()).strftime(
            timeformat
        ),
        "time_coverage_end": pd.to_datetime(nc_aggregated.TIME.values.max()).strftime(
            timeformat
        ),
...
def generate_netcdf_output_filename(
    nc, facility_code, data_code, site_code, product_type, file_version
):
    """
...
def write_netCDF_aggfile(nc_aggregated, ncout_filename, encoding):
    ...
    nc_aggregated[variables_all].to_netcdf(
        ncout_filename, encoding=encoding, format="NETCDF4_CLASSIC"
    )
...
def PDresample_by_hour(df, function_dict, function_stats):
    ...
    for variable in varnames:
        ds_var = df[variable]
        ds_var_resample = ds_var.resample(
            "1H", base=0.5
        )  # shift by half hour to centre bin on the hour
...
def hourly_aggregator(
    ...
    files_to_aggregate, bad_files = check_files(
        files_to_aggregate, site_code, parameter_names_accepted, input_dir=input_dir
    )
    ...
    for file_index, file in enumerate(files_to_aggregate):
        ...
        with xr.open_dataset(
            os.path.join(input_dir, file), mask_and_scale=True, decode_times=True
        ) as nc:
            parameter_names = list(
                set(list(nc.variables)) & set(parameter_names_accepted)
            )
            ...
            df_temp = PDresample_by_hour(
                df_temp, function_dict, function_stats
            )  # do the magic
            df_temp["instrument_index"] = np.repeat(file_index, len(df_temp)).astype(
                np.int32
            )
            df_data = pd.concat(
                [df_data, df_temp.reset_index()], ignore_index=True, sort=False
            )
    ...
    nc_metadata = xr.Dataset(
        {
            "LONGITUDE": (["INSTRUMENT"], df_metadata["LONGITUDE"].astype("float64")),
            "LATITUDE": (["INSTRUMENT"], df_metadata["LATITUDE"].astype("float64")),
            "NOMINAL_DEPTH": (
                ["INSTRUMENT"],
                df_metadata["NOMINAL_DEPTH"].astype("float32"),
            ),
            "instrument_id": (
                ["INSTRUMENT"],
                df_metadata["instrument_id"].astype("|S256"),
            ),
    ...
    add_attribute = {
        "rejected_files": "\n".join(list(bad_files)),
        "included_values_flagged_as": ", ".join(
            [qcflags_names[flag] for flag in qcflags]
        ),
        "generating_code_version": __version__,
    }
    add_attribute.update(
        utils.get_contributors(files_to_aggregate, input_dir=input_dir)
    )

    nc_aggregated.attrs = set_globalattr(
        nc_aggregated, TEMPLATE_JSON, site_code, add_attribute, parameter_names
    )
    nc_aggregated.attrs["abstract"] = nc_aggregated.attrs["abstract"].format(
        site_code=site_code, flags=", ".join(qcflags_names[flag] for flag in qcflags)
    )
    if 0 in qcflags:
        nc_aggregated.attrs["lineage"] += (
            "The percentage of quality controlled values used in the aggregation is "
    ...
    variable_attributes["PRES_REL"].update(
        {"applied_offset_by_instrument": applied_offset}
    )
    if download_url_prefix or opendap_url_prefix:
        variable_attributes["source_file"].update(
            utils.source_file_attributes(download_url_prefix, opendap_url_prefix)
        )

    time_units = variable_attributes["TIME"].pop("units")
    time_calendar = variable_attributes["TIME"].pop("calendar")
    encoding = {
        "TIME": {"_FillValue": None, "units": time_units, "calendar": time_calendar},
    ...
    for variable in parameter_names_all:
        ...
        nc_aggregated[variable].attrs = variable_attributes[variable]
        nc_aggregated[variable].attrs["long_name"] = (
            function_dict[variable] + " " + nc_aggregated[variable].attrs["long_name"]
        )
        nc_aggregated[variable].attrs.update(
            {
                "cell_methods": "TIME:"
                + function_dict[variable]
                + " (interval: 1 hr comment: time mid point)"
            }
        )
        ...
        for stat_method in function_stats:
            ...
            if stat_method == "count":
                ...
                nc_aggregated[variable_stat_name].attrs["units"] = variable_attributes[
                    variable
                ]["units"]

            if "standard_name" in nc_aggregated[variable].attrs:
                nc_aggregated[variable_stat_name].attrs[
                    "standard_name"
                ] = nc_aggregated[variable].attrs["standard_name"]
                nc_aggregated[variable + "_count"].attrs["standard_name"] = (
                    nc_aggregated[variable].attrs["standard_name"]
                    + " number_of_observations"
                )

            nc_aggregated[variable_stat_name].attrs["long_name"] = (
                stat_method + " data value in the bin, after rejection of flagged data"
            )
            nc_aggregated[variable_stat_name].attrs["cell_methods"] = (
                "TIME:" + stat_method
            )
            nc_aggregated[variable_stat_name].attrs["_FillValue"] = fill_value
            nc_aggregated[variable_stat_name] = nc_aggregated[
                variable_stat_name
            ].fillna(fill_value)
        nc_aggregated[variable].attrs.update(
            {"ancillary_variables": " ".join(ancillary_variables_attr)}
        )
    ...
    file_version = 2
    ncout_filename = generate_netcdf_output_filename(
        nc=nc_aggregated,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-files",
        dest="filenames",
        help="name of the file that contains the source URLs",
        required=True,
    )
    parser.add_argument(
        "-qc",
        dest="qcflags",
        help="list of QC flags to select variable values to keep",
        nargs="+",
        required=True,
    )
    parser.add_argument(
