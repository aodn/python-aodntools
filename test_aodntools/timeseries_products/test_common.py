...
BAD_TZ_FILE = os.path.join(
    TEST_ROOT,
    "IMOS_ANMN-NRS_TZ_20181213T080000Z_NRSROT_FV00_NRSROT-1812-SBE39-43_END-20181214T004000Z_C-20190827T000000Z.nc",
)
BAD_V_FILE = os.path.join(TEST_ROOT, "IMOS_ANMN-NRS_BAD_VELOCITY_FILE.nc")
AM_FILE = os.path.join(
    TEST_ROOT,
    "IMOS_ANMN-AM_GST_20190419T100000Z_NRSMAI_FV01_NRSMAI-CO2-1904-delayed_END-20190531T020000Z_C-20200625T000000Z.nc",
)
...
class TestQCVariableFunctions(unittest.TestCase):
    ...
    def test_check_flag_convetions(self):
        with xr.open_dataset(GOOD_TZ_FILE) as nc:
            ...
            self.assertEqual(
                check_imos_flag_conventions(nc, ["NONE"]), ["variable NONE not in file"]
            )
            self.assertEqual(
                check_imos_flag_conventions(nc, ["TEMP"]),
...
class TestCheckFile(unittest.TestCase):
    ...
    def test_wrong_site_and_var(self):
        ...
        self.assertEqual(
            set(error_list), {"Wrong site_code: NRSROT", "no variables to aggregate"}
        )
