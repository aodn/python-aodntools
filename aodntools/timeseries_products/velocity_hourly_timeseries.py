...
def append_resampled_values(nc_cell, ds, slice_start, binning_functions):
    ...
    time_slice = (
        np.fromiter(df_cell_1H.min().index, dtype="M8[ns]") - TIME_EPOCH
    ) / ONE_DAY
    ...
    (
        ...
        ds["DEPTH"][slice_start:slice_end],
    ) = cell_velocity_resample(df_cell_1H, "mean")
    ...
    for method in binning_functions:
        (
            ...
            ds["DEPTH_" + method][slice_start:slice_end],
        ) = cell_velocity_resample(df_cell_1H, method)

    return slice_end


## MAIN FUNCTION
def velocity_hourly_aggregated(
    files_to_agg,
    site_code,
    input_dir="",
    output_dir="./",
    ...
    SECONDS_TO_MIDDLE = ds.createVariable(
        varname="SECONDS_TO_MIDDLE", **inst_float_template
    )
    ...
    for index, file in enumerate(files_to_agg):
        ...
        with xr.open_dataset(os.path.join(input_dir, file)) as nc:
            ...
            ## in water only
            chunk_start = max(
                np.datetime64(nc.attrs["time_deployment_start"]), nc.TIME.data.min()
            )
            chunk_end = min(
                np.datetime64(nc.attrs["time_deployment_end"]), nc.TIME.data.max()
            )
            ...
            while chunk_start < chunk_partial and chunk_start <= chunk_end:
                nc_chunk = nc.where(
                    (nc.TIME >= chunk_start) & (nc.TIME < chunk_partial), drop=True
                )
                if is_2D:
                    ...
                    for cell_idx, cell_height in enumerate(heights):
                        ...
                        slice_end = append_resampled_values(
                            nc_cell[varlist_nc], ds, slice_start, binning_fun
                        )
                        CELL_INDEX[slice_start:slice_end] = np.full(
                            slice_end - slice_start, cell_idx, dtype=np.uint32
                        )

                        slice_start = slice_end
                else:
                    slice_end = append_resampled_values(
                        nc_chunk[varlist_nc], ds, slice_start, binning_fun
                    )
                    CELL_INDEX[slice_start:slice_end] = np.full(
                        slice_end - slice_start, 0, dtype=np.uint32
                    )
            ...
            ## metadata variables
            instrument_index[slice_instrument_start:slice_end] = np.repeat(
                index, slice_end - slice_instrument_start
            )
            ...
            instrument_id[index] = stringtochar(
                np.array(utils.get_instrument_id(nc), dtype="S256")
            )
    ...
    add_attribute = {
        "title": (
            "Long Timeseries Velocity Hourly Aggregated product: "
            + ", ".join(varlist)
            + " at "
            + site_code
            + " between "
    ...
    add_attribute.update(
        utils.get_contributors(files_to_agg=files_to_agg, input_dir=input_dir)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Concatenate X,Y,Z velocity variables from ALL instruments from ALL deployments from ONE site"
    )
    parser.add_argument(
