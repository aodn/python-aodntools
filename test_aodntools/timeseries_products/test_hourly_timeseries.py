...
class TestHourlyTimeseries(BaseTestCase):
    EXPECTED_OUTPUT_FILE = os.path.join(
        TEST_ROOT,
        "IMOS_ANMN-NRS_STZ_20181213_NRSROT_FV02_hourly-timeseries_END-20190523_C-20220428.nc",
    )

    def test_hourly_aggregator(self):
        output_file, bad_files = hourly_aggregator(
            files_to_aggregate=INPUT_PATHS,
            site_code="NRSROT",
            qcflags=(1, 2),
            output_dir="/tmp",
        )
        ...
        self.assertSetEqual(
            set(dataset.dimensions), {"OBSERVATION", "INSTRUMENT", "string256"}
        )
        self.assertTrue(
            set(dataset.variables.keys()).issubset(OBS_VARIABLES | INST_VARIABLES)
        )

        inst_variables = {
            n for n, v in dataset.variables.items() if v.dimensions[0] == "INSTRUMENT"
        }
        self.assertSetEqual(inst_variables, INST_VARIABLES)
        obs_variables = {
            n for n, v in dataset.variables.items() if v.dimensions == ("OBSERVATION",)
        }
    ...
    def test_some_files_without_good_data(self):
        output_file, bad_files = hourly_aggregator(
            files_to_aggregate=PH100_FILES,
            site_code="PH100",
            qcflags=(1, 2),
            input_dir=TEST_ROOT,
            output_dir="/tmp",
    ...
    def test_bad_timestamps(self):
        output_file, bad_files = hourly_aggregator(
            files_to_aggregate=SYD100_FILES,
            site_code="SYD100",
            qcflags=(1, 2),
            input_dir=TEST_ROOT,
            output_dir="/tmp",
